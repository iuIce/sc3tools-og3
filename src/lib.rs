...
pub fn run() -> Result<(), Box<dyn Error>> {
    ...
    let game_defs_file = {
        ...
        if let Ok(mut exe_path) = std::env::current_exe() {
            exe_path.pop();
            let local_path = exe_path.join("resources").join("gamedefs.json");
            if let Ok(local_data) = std::fs::read(&local_path) {
                data = Some(Cow::Owned(local_data));
            }
        }
        if data.is_none() {
            let local_path = "resources/gamedefs.json";
            if let Ok(local_data) = std::fs::read(local_path) {
                data = Some(Cow::Owned(local_data));
            }
        }
        data.unwrap_or_else(|| gamedef::ResourceDir::get("gamedefs.json").unwrap())
    };
    ...
    let supported_games: Vec<String> = defs.iter()
        .flat_map(|v| v.aliases.iter().cloned())
        .collect();
...
fn replace_text(
    ...
    let lines = script
        ...
        .zip_longest(txt.lines().map(|res| res.map(|s| {
            let has_vtext = s.contains("<vtext>");
            let s = s.replace("<vtext>", "[linebreak]");
            CozString(s.into(), has_vtext)
        })));
            ...
            for pair in scr_line.iter().zip_longest(txt_line.iter()) {
                match pair {
                    EitherOrBoth::Both(sc3, coz) => {
                        let eq = equivalent(&sc3?, &coz, gamedef, keep_fullwidth_chars, txt_line.1)
                    ...
                    EitherOrBoth::Right(coz) => {
                        let _ = sc3::StringToken::deserialize(&coz, gamedef, false, txt_line.1)
    ...
    let process_change = |i, s| {
        ...
        for tk in orig.iter() {
            ...
            if let sc3::StringToken::Text(text) = tk {
                ...
                fullwidth = decoded.iter(&gamedef.encoding_maps, false).any(|ch| {
...
fn equivalent(
    ...
    Ok(
        if let Ok(txt_tk) = sc3::StringToken::deserialize(txt_seg, gamedef, false, has_vtext) {
            *scr_tk == txt_tk
        } else {
            false
        },
