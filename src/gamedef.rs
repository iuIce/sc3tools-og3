...
pub fn get_resource(path: &str) -> Cow<'static, [u8]> {
    if let Ok(mut exe_path) = std::env::current_exe() {
        exe_path.pop();
        let local_path = exe_path.join("resources").join(path);
    ...
    ResourceDir::get(path).unwrap_or_else(|| panic!("Resource not found: {}", path))
}
...
impl GameDef {
    pub fn new(
        ...
        let charset: Cow<[u8]> =
            get_resource(&file_path(resource_dir, "charset.utf8"));
        let charset_str = std::str::from_utf8(charset.as_ref()).unwrap();
        let charset: Vec<String> = if charset_str.contains('\n') {
            charset_str.lines().map(|s| s.to_owned()).collect()
        } else {
            charset_str.chars().map(|c| c.to_string()).collect()
        };
        let compound_chars: Cow<[u8]> =
            get_resource(&file_path(resource_dir, "compound_chars.map"));
        let compound_chars_str = std::str::from_utf8(compound_chars.as_ref()).unwrap();
        let compound_chars = parse_compound_ch_map(compound_chars_str);
...
impl<'a> PuaMapping<'a> {
    ...
    pub fn parse(i: &str) -> IResult<&str, PuaMapping<'_>> {
