...
pub struct CozString<'a>(pub Cow<'a, str>, pub bool);
...
impl<'a> Sc3String<'_> {
    pub fn serialize(
        ...
        Ok(CozString(Cow::from(buf), false))
    }
    ...
    pub fn deserialize(
        ...
        for seg in s.iter() {
            let tk = StringToken::deserialize(&seg, gamedef, convert_to_fullwidth, s.1)?;
...
impl<'a> CozString<'a> {
    pub fn iter(&self) -> CozStringIter<'_> {
...
impl<'a> StringSegment<'a> {
    pub fn parse(input: &'a str) -> (&'a str, Self) {
        alt((
            map(Self::tag, StringSegment::Tag),
            map(Self::text, |s| {
    ...
    fn tag(i: &str) -> IResult<&str, Tag<'_>> {
...
impl<'a> StringToken<'a> {
    ...
    pub fn deserialize(
        ...
        match seg {
            StringSegment::Text(s) => text::encode_str(s, gamedef, convert_to_fullwidth, has_vtext)
...
mod tests {
    ...
    fn token_serialization_roundtrip() {
        ...
        assert_eq!(
            StringToken::deserialize(&text, &gamedef, false, false)
    ...
    fn serialization_roundtrip() -> Result<(), Box<dyn error::Error>> {
        let gamedef = gamedef::get_by_alias(&DEFS, "oregairu").unwrap();
        let src = CozString(Cow::from("[name]LuLu[line]Hi I am LuLu"), false);
    ...
    fn test_error(text: &str, f: impl FnOnce(Result<StringToken, Error>) -> bool) {
        ...
        let res = StringToken::deserialize(&seg, &gamedef, false, false);
        assert!(f(res));
    }
